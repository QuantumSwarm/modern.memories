...
export default function Navbar() {
  ...
  const handleAnchorClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    // Otherwise let Next.js handle page navigation
  }
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link
              href="/rsvp"
              className="px-5 py-2 bg-pink-500 hover:bg-pink-400 text-white text-sm font-medium rounded-full transition-all hover:scale-105"
            >
              Reserve Seat
            </Link>
          </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <Link
              href="/rsvp"
              onClick={() => setIsMobileMenuOpen(false)}
              className="px-8 py-3 bg-pink-500 hover:bg-pink-400 text-white font-medium rounded-full transition-all"
            >
              Reserve Seat
            </Link>
          </motion.div>
